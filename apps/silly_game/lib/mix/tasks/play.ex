defmodule Mix.Tasks.Play do
  ...
  defp receive_loop(
    ...
    receive do
      ...
      {^ref, _message} ->
        ...
        {:ok, conn} =
          Mint.WebSocket.stream_request_body(conn, ws_ref, data)
      ...
      message ->
        case Mint.WebSocket.stream(conn, message) do
          {:ok, conn, [{:data, ^ws_ref, data}]} ->
            {:ok, websocket, [{:text, text}]} =
              Mint.WebSocket.decode(websocket, data)

            handle_text(text)
            receive_loop(conn, websocket, ws_ref, prompt_task)
  ...
  defp handle_text("ping") do
    Mix.shell().info("PING! Press enter within 1s!")
  end

  defp handle_text("expired") do
    Mix.raise("You were too slow.")
  end

  defp handle_text("early") do
    Mix.raise("You were too early, don't cheat.")
  end
